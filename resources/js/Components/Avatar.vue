...
<template>
    <div class="relative" :class="{
        ...
        <div class="p-1.5 flex items-center justify-center rounded-full text-white h-full w-full cursor-pointer" :class="[
        user.color,
        {
            'text-xs': size == 'sm',
            'text-lg': size == 'md',
        ...
        <div v-if="hoverEnabled" v-show="showAvatarDetail"
            class="absolute top-0 w-[210px] max-w-lg mt-5 z-30 -translate-x-1/2 translate-y-3 left-1/2">
            <div v-show="showAvatarDetail"
                class="w-[full] h-auto bg-white p-5 flex flex-col rounded-md shadow-sm border border-secondary-300/70">
                <div class="mx-auto">
